...
int backgroundMode(char **arguments)
{
    int i = 0;
    while (arguments[i] != NULL)
    {
        int isAmpersand  = strcmp(arguments[i], "&") == 0;
        int isNull       = arguments[i + 1] == NULL;
        int isBackground = isAmpersand && isNull;
        int isError      = isAmpersand && !isNull;
        if (isBackground)
        {
            arguments[i] = NULL;
        ...
        {
            perror("Syntax error: & in uncorrect place");
            exit(SYNTAX_ERROR);
        }
        ++i;
    }
...
int runCommand(char **arguments, int mode, int background)
{
    int pid = -1;
    if (background)
        signal(SIGINT, SIG_IGN);
...
{
    int commandDirectory, commandExit;
    int backgroundFlag = 0, pid = -1, status = 0;
    commandDirectory = strcmp(arguments[0], "cd");
    commandExit      = strcmp(arguments[0], "exit");
    
    if (commandExit == 0)
        return 1;
    else if ((commandDirectory == 0) && (changeDirectory(arguments) == 1))
            perror("Error in directory change...");
    else
    {
        backgroundFlag = backgroundMode(arguments);
        if (backgroundFlag)
        {
            switch (pid = fork())
            {
                case 0:
                {
                    status = runCommand(arguments, isPipeRedirect, 1);
                    kill(getpid(), SIGKILL);
                }
                case -1:
                    return 1;
            ...
            status = runCommand(arguments, isPipeRedirect, 0);
        wait(NULL);
    }
    return status;
}
